        ...
        <aside class="fixed inset-y-0 left-0 z-50 bg-stone-950 text-white transition-all duration-500 ease-in-out border-r border-white/5 flex flex-col"
            ...
            <div class="h-20 flex items-center border-b border-white/5 transition-all duration-300 px-6"
                ...
                <div class="flex items-center justify-center gap-4 w-full overflow-hidden relative">
                    <span class="font-heading font-black text-2xl text-white whitespace-nowrap transition-all duration-300 uppercase tracking-tight text-center"
            ...
            <nav class="flex-1 overflow-y-auto py-8 px-4 space-y-3 no-scrollbar">
                ...
                <a href="{{ route($routePrefix . '.dashboard') }}" 
                   class="flex items-center gap-4 px-4 py-3.5 rounded-2xl transition-all duration-300 group {{ request()->routeIs($routePrefix . '.dashboard') ? 'bg-white text-stone-900 shadow-xl' : 'text-stone-400 hover:text-white hover:bg-white/5' }}"
                   :class="(!isMobile && !sidebarOpen) ? 'justify-center !px-0' : ''">
                ...
                @if($role === 'kasir')
                    <a href="{{ route('cashier.tables') }}" 
                       class="flex items-center gap-4 px-4 py-3.5 rounded-2xl transition-all duration-300 group {{ request()->routeIs('cashier.tables') ? 'bg-white text-stone-900 shadow-xl' : 'text-stone-400 hover:text-white hover:bg-white/5' }}"
                       :class="(!isMobile && !sidebarOpen) ? 'justify-center !px-0' : ''">
                        <div class="w-6 h-6 flex items-center justify-center shrink-0">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"></path></svg>
                        </div>
                        <span class="whitespace-nowrap overflow-hidden transition-all duration-300 font-bold text-[10px] uppercase tracking-[0.2em]" 
                              :class="(isMobile && mobileMenu) || (!isMobile && sidebarOpen) ? 'opacity-100 max-w-xs' : 'opacity-0 max-w-0 pointer-events-none'">Status Meja</span>
                    </a>
                @endif

                @if($role === 'barista')
                    <a href="{{ route('barista.menus') }}" 
                       class="flex items-center gap-4 px-4 py-3.5 rounded-2xl transition-all duration-300 group {{ request()->routeIs('barista.menus') ? 'bg-white text-stone-900 shadow-xl' : 'text-stone-400 hover:text-white hover:bg-white/5' }}"
                       :class="(!isMobile && !sidebarOpen) ? 'justify-center !px-0' : ''">
                        <div class="w-6 h-6 flex items-center justify-center shrink-0">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"></path></svg>
                        </div>
                        <span class="whitespace-nowrap overflow-hidden transition-all duration-300 font-bold text-[10px] uppercase tracking-[0.2em]" 
                              :class="(isMobile && mobileMenu) || (!isMobile && sidebarOpen) ? 'opacity-100 max-w-xs' : 'opacity-0 max-w-0 pointer-events-none'">Kontrol Stok</span>
                    </a>
                @endif

                @if($role === 'admin')
                    <a href="{{ route('admin.users.index') }}" 
                       class="flex items-center gap-4 px-4 py-3.5 rounded-2xl transition-all duration-300 group {{ request()->routeIs('admin.users.*') ? 'bg-white text-stone-900 shadow-xl' : 'text-stone-400 hover:text-white hover:bg-white/5' }}"
                       :class="(!isMobile && !sidebarOpen) ? 'justify-center !px-0' : ''">
                        <div class="w-6 h-6 flex items-center justify-center shrink-0">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"></path></svg>
                        </div>
                        <span class="whitespace-nowrap overflow-hidden transition-all duration-300 font-bold text-[10px] uppercase tracking-[0.2em]" 
                              :class="(isMobile && mobileMenu) || (!isMobile && sidebarOpen) ? 'opacity-100 max-w-xs' : 'opacity-0 max-w-0 pointer-events-none'">Pengguna</span>
                    </a>
                    <a href="{{ route('admin.tables.index') }}" 
                       class="flex items-center gap-4 px-4 py-3.5 rounded-2xl transition-all duration-300 group {{ request()->routeIs('admin.tables.*') ? 'bg-white text-stone-900 shadow-xl' : 'text-stone-400 hover:text-white hover:bg-white/5' }}"
                       :class="(!isMobile && !sidebarOpen) ? 'justify-center !px-0' : ''">
                        <div class="w-6 h-6 flex items-center justify-center shrink-0">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"></path></svg>
                        </div>
                        <span class="whitespace-nowrap overflow-hidden transition-all duration-300 font-bold text-[10px] uppercase tracking-[0.2em]" 
                              :class="(isMobile && mobileMenu) || (!isMobile && sidebarOpen) ? 'opacity-100 max-w-xs' : 'opacity-0 max-w-0 pointer-events-none'">Meja</span>
                    </a>
                    <a href="{{ route('admin.menus.index') }}" 
                       class="flex items-center gap-4 px-4 py-3.5 rounded-2xl transition-all duration-300 group {{ request()->routeIs('admin.menus.*') ? 'bg-white text-stone-900 shadow-xl' : 'text-stone-400 hover:text-white hover:bg-white/5' }}"
                       :class="(!isMobile && !sidebarOpen) ? 'justify-center !px-0' : ''">
                        <div class="w-6 h-6 flex items-center justify-center shrink-0">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"></path></svg>
                        </div>
                        <span class="whitespace-nowrap overflow-hidden transition-all duration-300 font-bold text-[10px] uppercase tracking-[0.2em]" 
                              :class="(isMobile && mobileMenu) || (!isMobile && sidebarOpen) ? 'opacity-100 max-w-xs' : 'opacity-0 max-w-0 pointer-events-none'">Menu</span>
                    </a>
                @endif
            </nav>
            ...
            <div class="p-4 border-t border-white/5">
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit" 
                            class="w-full flex items-center gap-4 px-4 py-3.5 rounded-2xl text-red-400 hover:bg-red-500/10 transition-all duration-300 group"
                            :class="(!isMobile && !sidebarOpen) ? 'justify-center !px-0' : ''">
                        <div class="w-6 h-6 flex items-center justify-center shrink-0">
